	...
	public function getlist()
	{
		...
		if ($article_sys['article_temp'] == 0) {
			...
			if ($article_sys['article_temp'] == 2) {
				...
				$articles = pdo_fetchall('SELECT a.id, a.article_title, a.resp_img, a.article_rule_credit, a.article_rule_money, a.article_author, a.article_date_v, a.article_readnum, a.resp_desc, a.article_category FROM ' . tablename('ewei_shop_article') . ' a left join ' . tablename('ewei_shop_article_category') . ' c on c.id=a.article_category WHERE a.article_state=1 and c.isshow=1 and a.uniacid=:uniacid ' . $where . ' order by a.displayorder desc, a.article_date_v desc limit ' . (($pindex - 1) * $psize . ',' . $psize), array(':uniacid' => $_W['uniacid']));
			}
		}
