<?php
//QQ；278638500  敷衍开发新版上传接口！  20181117   修改上传流程
if (!defined('IN_IA')) {
	exit('Access Denied');
}
define('EWEI_SHOPV2_NEW_AUTH_WXAPP', 'http://newapps.cwlwl.com/');
...
class Newrelease_EweiShopV2Page extends PluginWebPage
{
	...
	public function submit()
	{
		...
		$request = ihttp_post(EWEI_SHOPV2_NEW_AUTH_WXAPP . 'generate/upload?id=' , array('version' => $version, 'describe' => $describe, 'tabBar' => $tabBar, 'ticket' => $ticket,'appid' => $appid , 'site_id' => SITE_ID,'uniacid' => $_W['uniacid'], 'appIds' => $appIds));
      	if ($request['code'] != 200) {
